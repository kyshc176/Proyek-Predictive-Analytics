...
Original file is located at
    https://colab.research.google.com/drive/1aoRkHUiSCF2IMi9FcZurkjMDMrTUOc-o
...
- Matplotlib, Seaborn, Plotly: Untuk visualisasi data baik statis maupun interaktif.
- Scikit-learn (sklearn): Untuk preprocessing, pemodelan, evaluasi, dan pipeline machine learning.
- Imbalanced-learn (SMOTE): Untuk menangani permasalahan kelas tidak seimbang.
- Statsmodels (VIF): Untuk mendeteksi dan mengurangi multikolinearitas.
- XGBoost: Salah satu algoritma gradient boosting yang powerful untuk klasifikasi.
- Pengaturan tambahan juga dilakukan untuk:
- Menentukan style visualisasi (sns.set_style).
- Mengatur ukuran dan gaya default figure pada matplotlib.
"""
...
"""Pada tahap ini, dataset dimuat dari file bank_dataset.csv menggunakan pandas.read_csv."""
...
"""sample(5) untuk melihat 5 baris acak.
- Tujuannya adalah untuk mendapatkan gambaran umum struktur dan isi dataset, seperti jumlah kolom, nama kolom, tipe data, dan nilai-nilai awal dari data.

### **`Data Accessing`**

Tahap ini bertujuan untuk memahami struktur dataset dan melakukan pengecekan awal terhadap kualitas data. Proses yang dilakukan antara lain:

- shape: Mengetahui jumlah baris dan kolom.
- info(): Menampilkan informasi tipe data, jumlah non-null, dan nama kolom.
- duplicated().sum(): Menghitung jumlah duplikasi baris.
- isnull().sum() dan isna().sum(): Mengecek nilai yang hilang (missing values).
- describe(): Statistik deskriptif untuk kolom numerik, termasuk kuartil.
- value_counts() pada kolom y: Menampilkan distribusi target class.
- Langkah ini penting untuk mendeteksi potensi masalah seperti duplikasi, nilai kosong, dan ketidakseimbangan kelas sebelum dilakukan tahap preprocessing lebih lanjut.
"""
...
🔄 Menyalin dan Membersihkan Data untuk Analisis Lanjutan
- Dataset bank disalin ke variabel baru bank_df untuk menjaga data asli tetap utuh.
- Semua kolom bertipe object diubah menjadi category untuk menghemat memori dan meningkatkan efisiensi pemrosesan.
- Analisis dilakukan pada nasabah dengan saldo bank ≤ 0, untuk:
- - Mengetahui jumlah mereka.
- - Mengetahui berapa dari mereka yang tetap berlangganan deposito berjangka.
- - Menghitung persentase mereka berdasarkan latar belakang pendidikan dan pekerjaan (misalnya: primary, secondary, student, retired).
- Kemudian, baris-baris di mana nasabah memiliki saldo ≤ 0 namun tetap berlangganan deposito dihapus, karena dianggap anomali untuk analisis lebih lanjut.
"""
...
Pada tahap ini dilakukan eksplorasi data lebih lanjut, terutama terhadap fitur kategorikal dan numerik. Langkah-langkahnya:

- Menentukan kolom kategorikal dan numerik berdasarkan tipe data.
- Melakukan univariate analysis (satu variabel) menggunakan:
- - Histogram untuk melihat distribusi dari variabel target y.
- - Histogram untuk fitur job untuk melihat jumlah pelanggan per pekerjaan.
- - Pie chart untuk fitur kategorikal lainnya untuk melihat proporsi setiap kategori.
- Melakukan bivariate analysis (dua variabel) menggunakan:
- - Countplot untuk melihat hubungan antara fitur job dan y.
- - Countplot untuk melihat tren langganan deposito (y) per bulan (month).
- - Countplot untuk fitur kategorikal lainnya terhadap y.
- Visualisasi ini membantu memahami pola dan hubungan antara variabel independen dengan target y sebelum tahap pemodelan.
...
🧠 Tahap Feature Engineering dan Analisis Korelasi
Pada tahap ini dilakukan transformasi dan visualisasi lanjutan terhadap data dengan tujuan meningkatkan kualitas dataset untuk pemodelan. Langkah-langkahnya meliputi:

1. Visualisasi Outlier:
- Menggunakan boxplot pada fitur numerik age dan balance untuk mendeteksi nilai ekstrem (outliers).
2. Pembuatan Fitur Baru:
- job_categories: Mengelompokkan kategori pekerjaan (job) ke dalam 4 kelompok berdasarkan jenis pekerjaan.
- age_categories: Mengelompokkan usia menjadi kategori seperti struggling, stable, about to retire, dan lainnya untuk menggambarkan kondisi hidup nasabah.
3. Eksplorasi Fitur Baru terhadap Target:
- Visualisasi hubungan antara job_categories dan term_deposit menggunakan barplot.
- Visualisasi hubungan antara age_categories dan term_deposit menggunakan countplot dan scatterplot.
4. Transformasi Label Biner:
- Kolom default, loan, housing, dan term_deposit diubah menjadi nilai numerik 0 dan 1 untuk keperluan pemodelan.
5. Penyederhanaan Dataset:
- Menghapus kolom age, job, pdays, dan duration karena sudah direpresentasikan dalam fitur baru.
6. Analisis Korelasi:
- Menggunakan heatmap korelasi antar fitur numerik untuk melihat hubungan antar variabel.
...
Pada tahap ini dilakukan proses persiapan akhir sebelum model dibangun. Berikut langkah-langkah yang dilakukan:

1. One-Hot Encoding (Dummy Variables):
- Semua fitur kategorikal seperti marital, contact, poutcome, month, education, job_categories, dan age_categories diubah menjadi representasi numerik menggunakan pd.get_dummies().
- Hal ini memungkinkan model machine learning untuk memproses data kategorikal.
2. Penggabungan dan Penghapusan Kolom Asli:
- Kolom asli kategori dihapus setelah proses encoding untuk mencegah duplikasi informasi.
- Beberapa kolom dummy juga dihapus (drop satu kategori dari setiap fitur dummy) untuk menghindari multikolinearitas (dummy variable trap).
3. Pemisahan Fitur dan Target:
- x: variabel independen (fitur)
- y: variabel target, yaitu term_deposit (berlangganan deposito atau tidak)
4. Penyeimbangan Dataset (SMOTE):
- Karena data target tidak seimbang (biasanya lebih banyak pelanggan yang tidak berlangganan), digunakan SMOTE (Synthetic Minority Over-sampling Technique) untuk membuat distribusi target menjadi seimbang.
- Ini membantu model tidak bias terhadap kelas mayoritas.
5. Split Dataset:
- Dataset hasil resampling dipisah menjadi data latih (80%) dan data uji (20%) menggunakan train_test_split().
- Ini untuk mengevaluasi model secara adil pada data yang belum pernah dilihat selama pelatihan.
6. Fungsi Evaluasi Kinerja Model:
- Dibuat fungsi performance_metrics() untuk menampilkan:
- - Akurasi data latih dan uji
- - Classification report (precision, recall, f1-score)
- - Confusion matrix dalam bentuk heatmap
- - Fungsi ini reusable untuk semua model klasifikasi.
7. Preprocessing Pipeline:
- Dua versi ColumnTransformer disiapkan:
- - step1: menggunakan StandardScaler untuk standardisasi (mean = 0, std = 1)
- - step0: menggunakan MinMaxScaler untuk normalisasi (nilai diubah ke rentang [0, 1])
- Pemisahan ini berguna untuk eksperimen, karena beberapa model lebih cocok dengan data yang ternormalisasi, sementara yang lain dengan data terstandarisasi.
...
Setelah melakukan preprocessing dan pembagian data, kita melatih beberapa model klasifikasi untuk memprediksi apakah nasabah akan berlangganan deposito berjangka (term_deposit = yes).

Model yang Digunakan:
1. K-Nearest Neighbors (KNN):
- Hyperparameter tuning dilakukan menggunakan GridSearchCV untuk mencari nilai k terbaik (jumlah tetangga terdekat) berdasarkan skor recall.
- Pipeline mencakup imputasi nilai hilang (SimpleImputer), standardisasi (StandardScaler), dan pemodelan.
2. Support Vector Machine (SVM):
- Model SVM digunakan dengan preprocessing yang sama.
- Cocok untuk dataset dengan banyak fitur dan non-linear boundary.
3. Random Forest Classifier:
- Menggunakan GridSearchCV untuk mencari kombinasi terbaik antara jumlah pohon (n_estimators) dan kedalaman maksimal (max_depth).
- Model ini menangani data imbalance dan variabel kategorik dengan baik.
4. Evaluasi Model:
- Evaluasi dilakukan dengan fungsi performance_metrics() yang mencetak:
- - Akurasi data latih & uji
